...
namespace System.Buffers
{
    public readonly partial struct ReadOnlySequence<T>
    {
        ...
        internal bool TryGetBuffer(in SequencePosition position, out ReadOnlyMemory<T> memory, out SequencePosition next)
        {
            ...
            {
                Assert(positionObject is ReadOnlySequenceSegment<T>);
            ...
            else
            {
                ...
                {
                    Assert(positionObject is string);
                ...
                {
                    Assert(type == SequenceType.MemoryManager);
                    Assert(positionObject is MemoryManager<T>);
        ...
        {
            Assert(currentSegment != null); // currentSegment parameter is marked as nullable as the parameter is reused/reassigned in the body
            Assert(offset >= 0);
        ...
        private void BoundsCheck(in SequencePosition position, bool positionIsNotNull)
        {
            ...
            else
            {
                ...
                {
                    Assert(position.GetObject() != null);
        ...
        {
            // _sequenceStart and _sequenceEnd must be well-formed
            Assert(start <= long.MaxValue);
            Assert(end <= long.MaxValue);
            Assert(start <= end);
