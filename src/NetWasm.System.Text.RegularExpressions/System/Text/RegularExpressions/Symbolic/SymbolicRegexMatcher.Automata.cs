...
namespace System.Text.RegularExpressions.Symbolic
{
    internal sealed partial class SymbolicRegexMatcher<TSet>
    {
        ...
        /// </summary>
        /// <remarks>The matcher is expanded by the single reactor, so ordinary array reads are sufficient.</remarks>
        ...
        /// </summary>
        /// <remarks>The matcher runs on NetWasm's single reactor, so a direct assignment publishes the new table.</remarks>
        private static void ArrayResizeAndPublish<T>(ref T[] array, int newSize)
        {
        ...
        private MatchingState<TSet> GetOrCreateState_NoLock(SymbolicRegexNode<TSet> node, uint prevCharKind, bool isInitialState = false)
        {
            ...
            if (!_stateCache.TryGetValue(key, out MatchingState<TSet>? state))
            {
                ...
                if (state.Id == _stateArray.Length)
                {
                    ...
                    ArrayResizeAndPublish(ref _stateArray, newsize);
                    ArrayResizeAndPublish(ref _dfaDelta, newsize << _mintermsLog);
                    ArrayResizeAndPublish(ref _stateFlagsArray, newsize);
                    ArrayResizeAndPublish(ref _nullabilityArray, newsize);
                }
        ...
        private int? CreateNfaState(SymbolicRegexNode<TSet> node, uint prevCharKind)
        {
            ...
            if (!_nfaIdByCoreId.TryGetValue(coreState.Id, out int nfaStateId))
            {
                ...
                if (nfaStateId == _nfaCoreIdArray.Length)
                {
                    ...
                    ArrayResizeAndPublish(ref _nfaCoreIdArray, newsize);
                    ArrayResizeAndPublish(ref _nfaDelta, newsize << _mintermsLog);
                    ArrayResizeAndPublish(ref _capturingNfaDelta, newsize << _mintermsLog);
                }
        ...
        private bool TryCreateNewTransition(MatchingState<TSet> sourceState, int mintermId, int offset, bool checkThreshold, long timeoutOccursAt, [NotNullWhen(true)] out MatchingState<TSet>? nextState)
        {
            ...
            if (targetState is null)
            {
                ...
                _dfaDelta[offset] = targetState.Id;
            }
        ...
        private (int, DerivativeEffect[])[] CreateNewCapturingTransition(int nfaStateId, int mintermId, int offset)
        {
            ...
            if (targets is null)
            {
                ...
                targets = targetsList.ToArray();
                _capturingNfaDelta[offset] = targets;
            }
