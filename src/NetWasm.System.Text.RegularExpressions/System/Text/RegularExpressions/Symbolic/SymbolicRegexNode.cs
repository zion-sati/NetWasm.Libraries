    ...
    internal sealed class SymbolicRegexNode<TSet> where TSet : IComparable<TSet>, IEquatable<TSet>
    {
        ...
        internal bool IsNullableFor(uint context)
        {
            ...
            bool WithCache(uint context)
            {
                ...
                // NetWasm executes Regex analysis on its single reactor. No concurrent
                // readers or writers can observe this cache, so a plain read is sufficient.
                byte b = _nullabilityCache[context];
                if (b != UndefinedByte)
                {
                    return b == TrueByte;
                }
                ...
                _nullabilityCache[context] = is_nullable ? TrueByte : FalseByte;
