...
namespace System.Text.RegularExpressions
{
    ...
    public partial class Regex
    {
        ...
        private RegexRunner? _runner;                         // cached runner
        ...
        internal Regex(string pattern, CultureInfo? culture)
        {
            ...
            // Keep the no-options overload on the interpreter-only construction path.
        }
        ...
        internal Regex(string pattern, RegexOptions options, TimeSpan matchTimeout, CultureInfo? culture)
        {
            ...
            // NetWasm has no dynamic-code backend. Compiled selects the interpreter, while
            // NonBacktracking selects the managed symbolic engine.
            factory = (options & RegexOptions.NonBacktracking) != 0
                ? new Symbolic.SymbolicRegexRunnerFactory(tree, options, matchTimeout)
                : new RegexInterpreterFactory(tree);
        }
        ...
        internal WeakReference<RegexReplacement?> RegexReplacementWeakReference =>
            field ??= new WeakReference<RegexReplacement?>(null);

        /// <summary>Legacy extensibility hook retained for API compatibility.</summary>
        ...
        protected void InitializeReferences()
        {
            ...
            // This method remains a no-op for compatibility with older derived Regex types.
        }
        ...
        /// <summary>Takes the cached runner or creates a new one.</summary>
        private RegexRunner TakeRunner()
        {
            var runner = _runner;
            _runner = null;
