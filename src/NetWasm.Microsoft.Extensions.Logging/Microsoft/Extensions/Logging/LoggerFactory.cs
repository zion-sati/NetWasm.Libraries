...
namespace Microsoft.Extensions.Logging
{
    ...
    public class LoggerFactory : ILoggerFactory
    {
        ...
        public LoggerFactory(IEnumerable<ILoggerProvider> providers, IOptionsMonitor<LoggerFilterOptions> filterOption, IOptions<LoggerFactoryOptions>? options = null, IExternalScopeProvider? scopeProvider = null)
        {
            ...
            foreach (object provider in (IEnumerable)providers)
            {
                AddProviderRegistration((ILoggerProvider)provider, dispose: false);
            }

            _changeTokenRegistration = filterOption.OnChange((options, _) => RefreshFilters(options));
        ...
        private (MessageLogger[] MessageLoggers, ScopeLogger[]? ScopeLoggers) ApplyFilters(LoggerInformation[] loggers)
        {
            ...
            foreach (LoggerInformation loggerInformation in loggers)
            {
                LoggerRuleSelector.Select(_filterOptions,
                    loggerInformation.Provider,
                    loggerInformation.ProviderName,
                    loggerInformation.ProviderAlias,
                ...
                messageLoggers.Add(new MessageLogger(loggerInformation.Logger, loggerInformation.Category, loggerInformation.ProviderName, minLevel, filter));
        ...
        private sealed class LoggerFactoryDebugView(LoggerFactory loggerFactory)
        {
            public List<ILoggerProvider> Providers
            {
                get
                {
                    var providers = new List<ILoggerProvider>(loggerFactory._providerRegistrations.Count);
                    foreach (ProviderRegistration registration in loggerFactory._providerRegistrations)
                    {
