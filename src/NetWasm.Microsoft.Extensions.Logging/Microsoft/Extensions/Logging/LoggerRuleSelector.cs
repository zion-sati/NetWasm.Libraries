...
namespace Microsoft.Extensions.Logging
{
    ...
    {
        public static void Select(LoggerFilterOptions options, ILoggerProvider provider, string? providerName,
            string? providerAlias, string category, out LogLevel? minLevel,
            out Func<string?, string?, LogLevel, bool>? filter)
        {
            ...
            {
                if (IsBetter(rule, current, provider, providerName, providerAlias, category))
                {
                    current = rule;
                }
            }
        ...
        private static bool IsBetter(LoggerFilterRule rule, LoggerFilterRule? current,
            ILoggerProvider provider, string? providerName, string? providerAlias, string category)
        {
            // Skip rules with inapplicable type or category
            if (rule.ProviderFilter != null)
            {
                if (!rule.ProviderFilter(provider))
                {
                    return false;
                }
            }
            else if (rule.ProviderName != null && rule.ProviderName != providerName && rule.ProviderName != providerAlias)
            {
                return false;
            }
        ...
        private static bool HasProvider(LoggerFilterRule? rule)
            => rule?.ProviderName != null || rule?.ProviderFilter != null;
    }
}
