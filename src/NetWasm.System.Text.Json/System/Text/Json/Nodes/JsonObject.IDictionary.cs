        ...
        private OrderedDictionary<string, JsonNode?> InitializeDictionary()
        {
            ...
            if (dictionary is null)
            {
                ...
                if (exchangedDictionary is null)
                {
                    ...
                    // NetWasm executes JSON DOM mutation on one reactor; publishing
                    // the materialized dictionary before clearing the lazy element
                    // is sufficient and avoids requiring a cross-thread fence.
                    _jsonElement = null;
