...
#if DEBUG && !NETWASM
                // NetWasm source-generated metadata validates extension-data shapes
                // without runtime generic reflection. Keep the upstream assertions
                // only for targets that provide the reflection APIs they require.
...
#else
                    // NetWasm does not ship JsonNode or runtime type-name metadata;
                    // source-generated dictionary extension-data support is handled
                    // by the concrete interface branches below.
#endif
                    if (jsonPropertyInfo.PropertyType == typeof(IReadOnlyDictionary<string, object>))
                    {
