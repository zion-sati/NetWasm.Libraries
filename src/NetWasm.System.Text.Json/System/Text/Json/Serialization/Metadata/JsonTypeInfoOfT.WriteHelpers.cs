...
#if NETWASM
                    serializationCount = ++_serializationCount;
#else
                    serializationCount = Interlocked.Increment(ref _serializationCount);
#endif
                    if (serializationCount == MinSerializationsSampleSize)
                    {
...
#if NETWASM
                        if (_canUseSerializeHandlerInStreamingState == 0)
                        {
