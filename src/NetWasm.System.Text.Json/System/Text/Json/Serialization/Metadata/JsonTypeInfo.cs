...
namespace System.Text.Json.Serialization.Metadata
{
    ...
    public abstract partial class JsonTypeInfo
    {
        ...
        internal JsonPropertyInfoList PropertyList
        {
            get
            {
                ...
                JsonPropertyInfoList CreatePropertyList()
                {
                    ...
                    JsonPropertyInfoList? result = _properties;
                    _properties ??= list;
        ...
        internal JsonUnionCaseInfoList UnionCaseList
        {
            get
            {
                ...
                JsonUnionCaseInfoList CreateUnionCaseList()
                {
                    ...
                    JsonUnionCaseInfoList? result = _unionCases;
                    _unionCases ??= list;
                    return result ?? list;
                }
            }
        ...
        private bool _typeClassifierResolutionPending;
        ...
        public ICustomAttributeProvider? ConstructorAttributeProvider
        {
            get
            {
                Func<ICustomAttributeProvider>? ctorAttrProviderFactory = ConstructorAttributeProviderFactory;
        ...
        private void ConfigureTypeClassifier()
        {
            ...
            JsonTypeClassifierFactory? factory = _typeClassifierFactory;
#if NETWASM
            // Runtime classifier discovery is outside the selected profile.
            _ = factory;
            _typeClassifier = null;
            _typeClassifierFactory = null;
            _typeClassifierResolutionPending = false;
            return;
#else
            ...
            if (factory is not null)
            {
                ...
                // Metadata is configured by a single reactor in this profile;
                // direct publication is sufficient and avoids threading-only
                // interlocked dependencies.
                if (_typeClassifier is null)
                {
                    _typeClassifier = classifier;
                }
            }
        ...
        private static void BuildUnionValueTypeMap(IList<JsonUnionCaseInfo> unionCases, JsonSerializerOptions options, JsonTypeInfo target)
        {
            ...
            throw new PlatformNotSupportedException(
                "Generated JSON union metadata is not available in this profile.");
#else
        ...
        private bool _isAncestorPolymorphicTypeResolved;
...
#if NETWASM
            throw new NotSupportedException("Reflection-based JSON metadata is not supported; use source-generated metadata.");
#else
...
#if NETWASM
            throw new NotSupportedException("Reflection-based JSON property metadata is not supported; use source-generated metadata.");
#else
...
#if NETWASM
                StableSortProperties(propertyCache);
#else
...
#if NETWASM
            // Interface callback discovery is reflection-based. Generated metadata
            // supplies supported callbacks explicitly.
            return;
#else
        ...
        {
#if NETWASM
            return propertyType == typeof(IDictionary<string, object>) ||
                ...
                // JsonObject is the mutable DOM extension-data shape. Keep this
                // exact-type check reflection-free in the NetWasm profile.
                propertyType == typeof(Nodes.JsonObject);
#else
...
#if NETWASM
        private static void StableSortProperties(List<JsonPropertyInfo> properties)
        {
            // Keep source-generated property order stable without depending on
            // the reflection-only collection helpers from the desktop profile.
            for (int i = 1; i < properties.Count; i++)
            {
                JsonPropertyInfo item = properties[i];
                int j = i - 1;
                while (j >= 0 && properties[j].Order > item.Order)
                {
