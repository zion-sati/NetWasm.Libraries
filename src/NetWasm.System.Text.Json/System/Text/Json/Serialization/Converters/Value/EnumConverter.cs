...
namespace System.Text.Json.Serialization.Converters
{
    internal sealed class EnumConverter<T> : JsonPrimitiveConverter<T> // Do not rename FQN (legacy schema generation)
        where T : struct, Enum
    {
        private static readonly TypeCode s_enumTypeCode = EnumConverterFactory.Helpers.GetTypeCode<T>();

        // Odd type codes are conveniently signed types (for enum backing types).
        private static readonly bool s_isSignedEnum = ((int)s_enumTypeCode % 2) == 1;
        private static readonly bool s_isFlagsEnum = DetectFlagsEnum();
        ...
        /// </summary>
        private readonly Dictionary<ulong, JsonEncodedText> _nameCacheForWriting;

        /// <summary>
        /// Holds a mapping from input text to enum values including flag combinations and alternative casings.
        /// </summary>
        private readonly Dictionary<string, ulong> _nameCacheForReading;

        // Prevent flooding the cache due to exponential bitwise combinations of flags.
        // The cache is bounded by a soft limit to prevent flooding due to exponential bitwise combinations of flags.
        ...
        private unsafe bool TryParseEnumFromString(ref Utf8JsonReader reader, out T result)
        {
            ...
            Dictionary<string, ulong>.AlternateLookup<ReadOnlySpan<char>> lookup = _nameCacheForReading.GetAlternateLookup<ReadOnlySpan<char>>();
#else
            string source = ((ReadOnlySpan<char>)charBuffer).Trim().ToString();
            Dictionary<string, ulong> lookup = _nameCacheForReading;
#endif
        ...
        private static bool DetectFlagsEnum()
        {
            T[] values = Enum.GetValues<T>();
            for (int leftIndex = 0; leftIndex < values.Length; leftIndex++)
            {
                ulong left = ConvertToUInt64(values[leftIndex]);
                if (left == 0)
                {
                    continue;
                }
                ...
                {
                    ulong right = ConvertToUInt64(values[rightIndex]);
                    ulong combined = left | right;
                    if (right == 0 || combined == left || combined == right)
                    {
                        continue;
                    }

                    object value = Enum.ToObject(typeof(T), combined);
                    string flags = Enum.Format(typeof(T), value, "F");
                    string general = Enum.Format(typeof(T), value, "G");
                    if (flags.Contains(','))
                    {
        ...
        private static bool IsInteger(ReadOnlySpan<char> value)
        {
            if (value.IsEmpty)
            {
                return false;
