...
namespace System.Text.Json.Serialization.Converters
{
    ...
    /// <summary>
    /// Defines an object converter that supports deserialization through the
    /// configured <see cref="JsonUnknownTypeHandling"/> representation.
    /// </summary>
    internal sealed class DefaultObjectConverter : ObjectConverter
    {
        public DefaultObjectConverter()
        {
            // JsonElement/JsonNode parsing is synchronous and needs the complete
            // value. Force read-ahead for streaming reader entry points.
        ...
        internal override bool OnTryRead(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options, scoped ref ReadStack state, out object? value)
        {
            ...
            if (options.UnknownTypeHandling == JsonUnknownTypeHandling.JsonElement)
            {
                ...
                // An object-valued unknown type can still carry reference metadata.
